document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderChart() {
        ...
        // Show skills gap tags (from localStorage gap data)
        if (gapsEl) {
            ...
            if (allGaps.length > 0) {
                ...
                gapsEl.innerHTML = sorted.map(([skill, count]) =>
                    `<span class="tag opportunity" style="display:inline-flex;align-items:center;gap:6px;">${skill}${count > 1 ? `<span style="background:rgba(0,0,0,0.3);border-radius:999px;padding:1px 7px;font-size:0.7rem;">${count}×</span>` : ''}</span>`
                ).join('');
            } else {
                gapsEl.innerHTML = '<span style="color: var(--text-secondary); font-size: 0.9rem; opacity: 0.6;">No gap data yet — run an analysis first.</span>';
            }
        }
        ...
        const labels = sessionData.map(h => h.job.length > 22 ? h.job.substring(0,22)+'…' : h.job);
        ...
        const accentColor = rootStyles.getPropertyValue('--accent').trim() || '#06b6d4';
        ...
        myChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Match Score (%)',
                    data: values,
                    backgroundColor: values.map(v => v >= 75 ? primaryColor + 'cc' : v >= 40 ? accentColor + '99' : '#f43f5e99'),
                    borderColor: values.map(v => v >= 75 ? primaryColor : v >= 40 ? accentColor : '#f43f5e'),
                    borderWidth: 2,
                    borderRadius: 8,
                    borderSkipped: false,
                }]
            ...
            options: {
                ...
                scales: {
                    ...
                    x: {
                        grid: { display: false },
                        ticks: { color: '#94a3b8', font: { family: "'Outfit', sans-serif", size: 12 }, maxRotation: 30, minRotation: 0 }
                    }
                }
            }
        });
